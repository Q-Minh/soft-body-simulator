...
class tetrahedral_fem_embedded_surface
    : public interpolated_embedded_surface_t<
          math::fem_interpolation_t<typename mechanics::linear_tetrahedral_fem_model_t::cell_type>>
{
  public:
          ...
          math::differentiable::polynomial_hat_basis_function_t<1>>>
{
  public:
    ...
    using mechanical_model_type = mechanics::differentiable::linear_tetrahedral_fem_model_t;
    using base_type             = interpolated_embedded_surface_t<interpolation_op_type>;
    ...
    tetrahedral_fem_embedded_surface(
        std::vector<Eigen::Vector3d> const& points,
        std::vector<index_type> const& indices,
        mechanical_model_type const* mechanical_model);

    // Accessors
    mechanical_model_type const* mechanical_model() const;
    index_type cell_containing_vertex(index_type vi) const;
  ...
  private:
    mechanical_model_type const* mechanical_model_;
    std::vector<index_type> cell_containing_vertex_;
};
