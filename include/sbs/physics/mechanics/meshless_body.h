...
class range_searcher_t : public Discregrid::KDTree<Discregrid::BoundingSphere>
{
  public:
    ...
    range_searcher_t();
    range_searcher_t(std::vector<meshless_node_t> const* nodes);
  ...
  private:
    std::vector<meshless_node_t> const* nodes_;
};

class meshless_body_t : public physics::body_t
{
  public:
    meshless_body_t(
    ...
    std::vector<meshless_node_t> const& nodes() const;
    physics::tetrahedral_mesh_boundary_t const& surface_mesh() const;
    physics::tetrahedral_mesh_boundary_t& surface_mesh();
    collision::point_bvh_model_t const& bvh() const;

    void initialize_physical_model(simulation_t const& simulation);

  private:
    std::vector<meshless_node_t> physical_model_;
    range_searcher_t
    ...
    physics::tetrahedral_mesh_boundary_t visual_model_;
